...
class GraphGenerator:
	...
	"""
	def __init__(self, sourceFilePath, levelspath, extracted, only_facct, nodeIdPath = None):
		self.node_levels = {}
		self.node_records = {}
	...
	def explore_nodeset(self):
		...
		for item in ijson.items(self.sourceFile, "item", use_float = True):
			...
			try:
				...
				if "references" in item:
					for reference in item["references"]:
						if reference in self.node_levels and self.node_levels[reference] == 0:		# the given reference refers to a FAccT paper
							if item_id not in self.node_levels:						# should not be a paper already in the network; eg: a FAccT paper citing another FAccT paper
								self.node_levels[item_id] = -1
			except KeyError:
				pass
		...
		with open(self.levelspath, "w") as levels_json:
		    json.dump(self.node_levels, levels_json)
	...
	def edge_list_facct(self):
		edge_list = []
		edgelistFile = os.path.join(os.path.dirname(self.sourceFilePath), "extracted_records_edgelist_v13.dat")
		with open(edgelistFile, 'w') as f_write:	
			for node_id, node_record in self.node_records.items():
				if self.node_levels[node_id] != 0:		# source node not in FAccT
					continue
				...
				if "references" in node_record:
					...
					for reference_id in references:
						if str(reference_id) in self.node_records:
							if self.node_levels[reference_id] != 0:			# target node not in FAccT
								continue
							edge_list.append((node_id, reference_id))
							f_write.write("{node_id}\t{reference_id}\n".format(node_id = node_id, reference_id = reference_id))


		print("FaCCT Edgelist: ", edge_list)
		return edge_list

	def top_k_stats(self, edge_list):
		incoming_dict = {}
		outgoing_dict = {}

		for edge in edge_list:						# edge direction: edge[0]->edge[1]
			if self.node_levels[edge[0]] == 0:		#edge from a FAccT paper
				if edge[1] not in outgoing_dict:
					outgoing_dict[edge[1]] = 1
				else:
					outgoing_dict[edge[1]] += 1
			if  self.node_levels[edge[1]] == 0:		#edge to a FAccT paper
				if edge[1] not in incoming_dict:
					incoming_dict[edge[1]] = 1
				else:
					incoming_dict[edge[1]] += 1

		incoming_counter = Counter(incoming_dict)
		outgoing_counter = Counter(outgoing_dict)

		top_10_incoming = incoming_counter.most_common(10)
		top_10_outgoing = outgoing_counter.most_common(10)

		print("Outgoing.............")
		for node_count in top_10_outgoing:
			print(self.node_records[node_count[0]]['title'], node_count[1])
		print("Incoming.............")
		for node_count in top_10_incoming:
			print(self.node_records[node_count[0]]['title'], node_count[1])
	...
	def generate_network(self):
		if self.extracted == True:
			...
			with open(self.levelspath) as levels_json:
			    self.node_levels = json.load(levels_json)
			print("Levels: ", self.node_levels)
		else:
			self.explore_nodeset()
			self.extract_records()

		if self.only_facct:
			edge_list = self.edge_list_facct()
		else:
			edge_list = self.edge_list()

		self.top_k_stats(edge_list)
...
class Partition:
	...
	algorithm_datapath: str
		...
		4. For Directed Louvain, this is the path that stores the DirectedLouvain implementation, i.e., it should contain the bin folder
	nodeview : dictionary
...
def main():
	...
	parser.add_argument('levelspath', type = str, help = "Path to store/retrieve the node levels json")
	...
	parser.add_argument('--only_facct', default= False, action = 'store_true', help = "Set to true to construct a graph only on FAccT articles [level 0]")
	...
	levelspath = args.levelspath
	algorithm = args.algorithm
	algorithm_datapath = args.algorithm_datapath
	only_facct = args.only_facct
	...
	G = GraphGenerator(sourceFilePath, levelspath, skip_extraction, only_facct, nodeIdPath)
